  ...
  socket.on('get_likes', function (data) {
    redisPublishClient.lrange(
      ...
      function (err, replies) {
          ...
          replies.forEach(function (reply, i) {
              ...
              redisPublishClient.hgetall(reply, function (err, obj) {
                ...
                if(data.user_id === obj.user_id){
                  ...
                  // io.to(socket.id).emit('block_like', replies.length);
                  tag = true;
        ...
        getTotal(function(tags) {
          var json = {tag:tag, replies:replies.length};
          io.sockets.emit('retrieve_likes', json);
    ...
    // redisPublishClient.lindex(
    //   "likes_list_"+ data.evidence_id, "like_" + data.evidence_id + "_" + data.user_id,
    //   function (err, reply) {
    //     console.log("like_" + data.evidence_id + "_" + data.user_id);
