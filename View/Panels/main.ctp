	...
	google.load("visualization", "1", {packages:["corechart"]});
      ...
      var options = {
        ...
        var chart = new google.visualization.PieChart(document.getElementById('piechart'));
      	...
      	google.load("visualization", "1", {packages:["corechart"]});
		google.setOnLoadCallback(drawCountChart);
		function drawCountChart() {
		var data = google.visualization.arrayToDataTable([
		  ['Month', 'Overall', 'Unique'],
		  ['Aug/2014', 
		  	<?= ($redis->get('August:2014:visitors') ? $redis->get('August:2014:visitors') : 0) ?>, 
		  	<?= ($redis->scard('August:2014:uniqueVisitors') ? $redis->scard('August:2014:uniqueVisitors') : 0) ?>],
		  ['Sep/2014', 
		  	<?= ($redis->get('September:2014:visitors') ? $redis->get('September:2014:visitors') : 0) ?>, 
		  	<?= ($redis->scard('September:2014:uniqueVisitors') ? $redis->scard('September:2014:uniqueVisitors') : 0) ?>],
		  ['Oct/2014', 
		  	<?= ($redis->get('October:2014:visitors') ? $redis->get('October:2014:visitors') : 0) ?>, 
		  	<?= ($redis->scard('October:2014:uniqueVisitors') ? $redis->scard('October:2014:uniqueVisitors') : 0) ?>],
		  ['Nov/2014', 
		  	<?= ($redis->get('November:2014:visitors') ? $redis->get('November:2014:visitors') : 0) ?>, 
		  	<?= ($redis->scard('November:2014:uniqueVisitors') ? $redis->scard('November:2014:uniqueVisitors') : 0) ?>]
		]);

		var options = {
		  title: "",
		  hAxis: {title: "<?= __('Month') ?>",  titleTextStyle: {color: '#333'}},
		  vAxis: {minValue: 0}
		};
